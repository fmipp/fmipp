...
comp->fmuData->modelData.realVarsData[0].attribute.start = 0.5;
...
fmiReal getReal(ModelInstance* comp, const fmiValueReference vr) {
  switch (vr) {
      ...
      case $Pp_ : return comp->fmuData->simulationInfo.realParameter[0]; break;
      default:
          return fmiError;
  }
}
...
fmiStatus setReal(ModelInstance* comp, const fmiValueReference vr, const fmiReal value) {
  switch (vr) {
      ...
      case $Pp_ : comp->fmuData->simulationInfo.realParameter[0]=value; break;
      default:
          return fmiError;
  }
  return fmiOK;
