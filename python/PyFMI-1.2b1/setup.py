...
# EW: Added variables to include FMIPP
fmipp_incdirs = ""
fmipp_libdirs = ""
fmipplib_shared = ""
...
for x in sys.argv[1:]:
    ...
    if not x.find('--fmil-home'):
        ...
        copy_args.remove(x)
    # EW: Added input parameter: FMIPP install dir.
    if not x.find('--fmipp-home'):
        fmipp_incdirs = O.path.join(x[13:],'include')
        fmipp_libdirs = O.path.join(x[13:],'lib')
        copy_args.remove(x)
...
if 0 != sys.argv[1].find("clean"): #Dont check if we are cleaning!
    if sys.platform.startswith("win"):
        ...
        # EW: Added check for FMIPP lib.
        files = O.listdir(O.path.join(fmipp_libdirs))
        for file in files:
            if "libfmipp" in file and not file.endswith("a"):
                shutil.copy2(O.path.join(fmipp_libdirs,file),O.path.join(".","src","pyfmi"))
                fmipplib_shared = O.path.join(".","src","pyfmi",file)
                break
        else:
            raise Exception("Could not find FMIPP library at: %s"%libdirs)
...
def check_extensions():
    ...
    # EW: Add defnitions for FMIPP.
    ext_list[-1].libraries += ["libfmipp"] # FMIPP library.
    ext_list[-1].include_dirs += [fmipp_incdirs] # FMIPP include dir.
    ext_list[-1].library_dirs += [fmipp_libdirs] # FMIPP lib dir.
    ext_list[-1].language="c++"
    ...
    # EW: Use of '-fpermissive' is in general discouraged. However, Cython's issues
    # with const variables leaves no other choice here. Defining FMI_ME_H prevents
    # the compiler to parse file 'fmi_me.h', which is of no use for the wrapper.
    ext_list[-1].extra_compile_args += [ "-DFMI_ME_H", "-fpermissive" ]
